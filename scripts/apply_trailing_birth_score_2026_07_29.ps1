# apply_trailing_birth_score_2026_07_29.ps1 -- aplica
# docs/SETUP_SUPABASE_TRAILING_BIRTH_SCORE_2026_07_29.sql via RPC exec_sql
# (mesmo padrao de scripts/apply_decision_reflections_2026_07_28.ps1).
# Roda uma vez via workflow_dispatch (job apply-trailing-birth-score-schema).
...
$sqlPath = Join-Path (Split-Path $PSScriptRoot -Parent) "docs\SETUP_SUPABASE_TRAILING_BIRTH_SCORE_2026_07_29.sql"
...
try {
    $testUrl = "$SupabaseUrl/rest/v1/trailing_state?select=pk_id,birth_score,birth_mesa_sinal,birth_fqs_category&limit=1"
    Invoke-RestMethod -Uri $testUrl -Headers $restHeaders -Method Get -ErrorAction Stop | Out-Null
    Write-Host "  trailing_state (birth_score cols) read OK -- schema cache reloaded successfully" -ForegroundColor Green
} catch {
...
try {
    ...
    $testRecord = @{
        ...
        birth_score = 78.5; birth_mesa_sinal = "schema check"; birth_fqs_category = "GEM"
    } | ConvertTo-Json
    Invoke-RestMethod -Uri $upsertUrl -Headers $upsertHeaders -Method Post -Body $testRecord -ErrorAction Stop | Out-Null
    Write-Host "  trailing_state upsert (birth_score cols) OK -- write path funcional" -ForegroundColor Green
