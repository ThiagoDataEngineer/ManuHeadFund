# apply_partial_exit_ladders_2026_07_31.ps1 -- aplica
# docs/SETUP_SUPABASE_PARTIAL_EXIT_LADDERS_2026_07_31.sql via RPC exec_sql
# (mesmo padrao de scripts/apply_gem_position_events_2026_07_29.ps1).
# Roda uma vez via workflow_dispatch (job apply-partial-exit-ladders-schema).
...
$sqlPath = Join-Path (Split-Path $PSScriptRoot -Parent) "docs\SETUP_SUPABASE_PARTIAL_EXIT_LADDERS_2026_07_31.sql"
...
try {
    $testUrl = "$SupabaseUrl/rest/v1/partial_exit_ladders?select=id&limit=1"
    Invoke-RestMethod -Uri $testUrl -Headers $restHeaders -Method Get -ErrorAction Stop | Out-Null
    Write-Host "  partial_exit_ladders read OK -- schema cache reloaded successfully" -ForegroundColor Green
} catch {
    Write-Host "  partial_exit_ladders read STILL FAILS: $_" -ForegroundColor Red
...
try {
    ...
    $insertUrl = "$SupabaseUrl/rest/v1/partial_exit_ladders"
    $testRecord = @{
        market = "TEST_SCHEMA_CHECK"; active = $false
        levels_json = "{}"
    } | ConvertTo-Json
    $inserted = Invoke-RestMethod -Uri $insertUrl -Headers $insertHeaders -Method Post -Body $testRecord -ErrorAction Stop
    Write-Host "  partial_exit_ladders insert OK -- write path funcional" -ForegroundColor Green

    $testId = $inserted[0].id
    $delUrl = "$SupabaseUrl/rest/v1/partial_exit_ladders?id=eq.$testId"
    ...
    Write-Host "  partial_exit_ladders insert FAILED: $_" -ForegroundColor Red
}
