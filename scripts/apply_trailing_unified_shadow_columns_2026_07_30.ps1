# apply_trailing_unified_shadow_columns_2026_07_30.ps1 -- aplica
# docs/ALTER_TRAILING_UNIFIED_SHADOW_ADD_COLUMNS_2026_07_30.sql via RPC exec_sql
# (mesmo padrao de scripts/apply_gem_position_events_2026_07_29.ps1).
# Roda uma vez via workflow_dispatch (job apply-trailing-unified-shadow-columns).
...
$sqlPath = Join-Path (Split-Path $PSScriptRoot -Parent) "docs\ALTER_TRAILING_UNIFIED_SHADOW_ADD_COLUMNS_2026_07_30.sql"
...
Write-Host "`nRe-testing read + upsert com as colunas novas:" -ForegroundColor Cyan
$restHeaders = @{
    "Authorization"   = "Bearer $ServiceKey"
    "apikey"          = $ServiceKey
...
try {
    $testUrl = "$SupabaseUrl/rest/v1/trailing_unified_shadow?select=id,trendline_factor,pushed_live&limit=1"
    Invoke-RestMethod -Uri $testUrl -Headers $restHeaders -Method Get -ErrorAction Stop | Out-Null
    Write-Host "  trailing_unified_shadow read (trendline_factor, pushed_live) OK -- schema cache reloaded" -ForegroundColor Green
} catch {
    Write-Host "  trailing_unified_shadow read STILL FAILS: $_" -ForegroundColor Red
...
try {
    ...
    $insertUrl = "$SupabaseUrl/rest/v1/trailing_unified_shadow"
    $testRecord = @{
        market = "TESTUSDT"; side = "LONG"; ts = [datetime]::UtcNow.ToString("o")
        real_stop = 1.0; unified_action = "HOLD"
        trendline_factor = 1.0; pushed_live = $false
    } | ConvertTo-Json
    $inserted = Invoke-RestMethod -Uri $insertUrl -Headers $insertHeaders -Method Post -Body $testRecord -ErrorAction Stop
    Write-Host "  trailing_unified_shadow insert OK -- write path funcional" -ForegroundColor Green

    $testId = $inserted[0].id
    $delUrl = "$SupabaseUrl/rest/v1/trailing_unified_shadow?id=eq.$testId"
    ...
    Write-Host "  trailing_unified_shadow insert FAILED: $_" -ForegroundColor Red
}
