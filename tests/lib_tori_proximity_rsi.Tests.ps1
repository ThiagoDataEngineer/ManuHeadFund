...
Describe "RSI Calculation - lib_tori_proximity (_ToriProx-CalcRSI)" {
    ...
    Context "Comparison with _CP-CalcRsiArray" {
        It "Should return same RSI as _CP-CalcRsiArray (last value)" {
            # Load lib_chart_patterns for comparison
            . "$root\agents\lib_chart_patterns.ps1"
            
            $closes = @(0..29 | ForEach-Object { 100 + $_ * 2 })
            
            $rsi_tori = _ToriProx-CalcRSI -Closes $closes -Period 14
            $rsi_cp = _CP-CalcRsiArray -Closes $closes -Period 14
