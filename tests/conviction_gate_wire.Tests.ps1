Describe "ConvictionGate Wire (Blocker #1 Fix)" {
    ...
    Context "Test-ConvictionGate with Mesa Score Override" {
        ...
        It "Should use conviction_threshold (50) when mesa <60" {
            # Mesa 50, conviction 51 should pass (51 > 50)
            $result = Test-ConvictionGate -conviction 51 -mesa_score 50
            $result | Should -Be $true
        }

        It "Should FAIL when mesa <60, conviction <50" {
            $result = Test-ConvictionGate -conviction 45 -mesa_score 50
            $result | Should -Be $false
        }
    }
