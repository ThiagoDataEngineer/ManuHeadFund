# MASTER TDD 2/3: Scanner Local Activation + Whitelist Rebuild
# Propósito: Ativar scanner, validar gates, detectar gargalos
# Status: PRODUÇÃO (semi-mock)

Describe "MASTER TDD 2: Scanner Local Activation + Whitelist" -Tags "scanner","critical" {
    ...
    Context "2.2: Whitelist Contents Validation" {
        ...
        It "Whitelist arquivo existe (conteúdo validado em TDD 1)" {
    ...
    Context "2.3: Mock Scan Cycle (1 discovery iteration)" {
        It "Mock: Resolve-MarketDiscovery retorna structure válida" {
            $mock_market = @{
                market = 'BCHUSD'
                score_pred = 82
                tier = 'tier_a_live'
    ...
    Context "2.4: Gate Rejection Detection (why nothing enters)" {
        It "Log 25/06 mostra razões de rejeição (FQS ausente, etc)" {
            $log = Get-Content 'logs/master_20260625.log' -Raw
            ($log -like '*FQS*' -or $log -like '*BETA*' -or $log -like '*TIER*') | Should Be $true
        }

        It "Rejeições são válidas: tier_B exige Mesa FORTE em BEAR (correto)" {
            $log = Get-Content 'logs/master_20260625.log' -Raw
    ...
    Context "2.6: Sanity Check (jornada sem bloqueios)" {
        ...
        It "Capital safety: 1% max por trade (validar)" {
            $capital = 1000
            $risk_per_trade = $capital * 0.01
            ($risk_per_trade -le 10) | Should Be $true
        }

        It "RR mínimo 1:5 (validar)" {
            $rr = 5.0
