...
# BeforeAll fora de Describe nao existe no Pester 3 ("BeforeAll may only be
# used inside a Describe block"), sintaxe -Should -Be tambem e Pester 5.
# Setup movido pro topo do script (Pester 3.x: variaveis de script sao
# visiveis nos It blocks sem precisar de BeforeAll).
...
$script:mockDecisionGrades = @(
    @{ direction = "SHORT"; regime = "BEAR_WEAK"; accuracy = 40; n = 507; would_win_pct = 57 }
    @{ direction = "LONG"; regime = "BULL_STRONG"; accuracy = 75; n = 21; would_win_pct = 68 }
)

$script:mockCounterfactual = @(
    @{ market = "LINKUSDT"; direction = "LONG"; avg_gain_pct = 79; n_skipped = 12; n_would_win = 8 }
...
$script:mockCapital = @(
    @{ balance_spot = 2425; balance_futures = 2741; deployed_pct = 0.65; margin_free_pct = 0.15; liquidation_price = 45000 }
)

$script:mockPositions = @(
    @{ market = "LINKUSDT"; side = "LONG"; entry_price = 10.25; qty = 50; size_usd = 512.5 }
)

$script:mockCapitalLowMargin = @(
    @{ margin_free_pct = 0.08; deployed_pct = 0.9; balance_spot = 100; balance_futures = 100; liquidation_price = 0 }
)

function Mock-SupabaseStateDefault {
    Mock -CommandName Get-SupabaseState -MockWith {
        switch ($Table) {
...
Describe "Get-TrailingHistoryEnrichment" {
    ...
    It "retorna MEDIUM confidence com n < 5" {
        $result = Get-TrailingHistoryEnrichment -Market "ETHUSDT" -Regime "BULL_WEAK" -Side "LONG"
...
Describe "Get-CapitalEnrichment" {
    ...
    It "sinaliza risco HIGH quando margin < 10%" {
        Mock -CommandName Get-SupabaseState -MockWith { return ,$script:mockCapitalLowMargin }
...
Describe "Get-MentorSupabaseEnrichment" {
    ...
    It "inclui todos os campos P0 e P1" {
        # 2026-07-23 FIX: mock so tem decision_grades p/ SHORT|BEAR_WEAK
        # (nao existe LONG|BEAR_WEAK) -- teste original nunca rodou de
        # verdade (BeforeAll fora de Describe quebrava no Pester 3) entao
        # essa combinacao inexistente nunca foi pega.
        $result = Get-MentorSupabaseEnrichment -Market "LINKUSDT" -Direction "SHORT" -Regime "BEAR_WEAK"
